...
## Model choices and Anomaly Detection

**One Class SVM**: My first instinct was to use a one-class support vector machines (OCSVM) to fit on a clean training dataset, then test on an evaluation dataset to predict outliers. However, the model didn't work for detecting anomalies, possibly because the data points in my time series were mostly flat lines of the same value that jumped up or down to other flat lines, which didnt offer enough variety for the model to be able to correctly fit and cluster the data. 
